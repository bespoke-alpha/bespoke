...
internalRegisterTransform({
    transform: emit => str => {
        ...
        str = str.replace(/(new Promise\(\()(\([a-zA-Z_\$][\w\$]*,[a-zA-Z_\$][\w\$]*\)=>\{var ([a-zA-Z_\$][\w\$]*)=([a-zA-Z_\$][\w\$]*)\.miniCssF\([a-zA-Z_\$][\w\$]*\),([a-zA-Z_\$][\w\$]*)=\4\.p\+\3)/, "$1async$2,$5=await __applyTransforms($5)");
        emit();
        return str;
    },
    glob: /^\/xpui\.js/,
...
export const applyTransforms = (path) => {

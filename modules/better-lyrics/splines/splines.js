...
import { matrixMultMatrix, remapScalar, scalarAddVector, scalarMultVector, vectorAddVector, vectorDist, vectorDivScalar, vectorMultVector, vectorSubVector, } from "../../delulib/math.js";
var EndCondition;
...
class Monomial {
    ...
    at(t, n = 0) {
        ...
        const weights = vectorDivScalar(_.range(n)
            .map(i => scalarAddVector(i + 1, powers))
            .reduce((u, v) => u.map((_, i) => u[i] * v[i]), new Array(powers.length).fill(1)), (t1 - t0) ** n);

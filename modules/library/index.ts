...
onTrackListMutationListeners.push(async (_, tracks) => {
    ...
    tracks.map(async (track, i) => {
		const uri = urisForIsrc[i]
		const urisForIsrc = await db.tracks.where("isrc").equals(isrcs[i]).uniqueKeys()
        setTrackGreyed(track, urisForIsrc.length > 1 && urisForIsrc.includes(uri))
    })
})
